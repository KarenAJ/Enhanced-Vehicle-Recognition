...
v = np.median(thresh_image)

#---- apply automatic Canny edge detection using the computed median----
lower = int(max(0, (1.0 - sigma) * v))
upper = int(min(255, (1.0 + sigma) * v))

wide = cv2.Canny(thresh_image, 10, 200)
tight = cv2.Canny(thresh_image, 225, 250)
auto = cv2.Canny(thresh_image,250,255)

canny_image = cv2.Canny(thresh_image,lower,upper)
cv2.namedWindow("Image after applying Canny",cv2.WINDOW_NORMAL)
# Creating a Named window to display image
cv2.imshow("Image after applying Canny", np.hstack([wide, tight, auto]))
# Display Image
canny_image = cv2.convertScaleAbs( np.hstack([wide, tight, auto]))
